...
jest.mock("@/data/utils", () => ({
  findMediaType: (src: string) =>
    src.includes(".mp4") ? MediaType.VIDEO : MediaType.IMAGE,
  MediaType: {
    IMAGE: "IMAGE",
    VIDEO: "VIDEO",
  },
...
describe("PageHero", () => {
  it("renders title correctly", () => {
    render(
      <PageHero title="Welcome to Our Church" media={{ src: "/image.jpg" }} />
    );
  ...
  it("renders an image when media type is IMAGE", () => {
    ...
    expect(
      screen.getByAltText("Decorative Background Image")
    ).toBeInTheDocument();
  });
  ...
  it("applies the correct height when short prop is true", () => {
    const { container } = render(
      <PageHero title="Hero" short media={{ src: "/image.jpg" }} />
    );
    expect(container.firstChild).toHaveClass("h-[60dvh]");
  });

  it("applies the correct height when short prop is false", () => {
    const { container } = render(
      <PageHero title="Hero" media={{ src: "/image.jpg" }} />
    );
    expect(container.firstChild).toHaveClass("h-[95dvh]");
  });
});
