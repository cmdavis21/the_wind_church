...
describe("CenterTextSection", () => {
  ...
  it("should highlight words correctly when highlight prop is passed", () => {
    ...
    highlightedWords.forEach((word) => {
      expect(word).toHaveClass("font-bold text-yellow dark:text-softYellow");
    });
  });

  it("should render title as React element", () => {
    const reactElementTitle = (
      <span className="text-blue">This is a React element title</span>
    );
    render(
      <CenterTextSection title={reactElementTitle} description={description} />
    );

    // Check if the React element title is rendered correctly
    expect(
      screen.getByText("This is a React element title")
    ).toBeInTheDocument();
  });
  ...
  it("should handle the highlight prop for words correctly", () => {
    ...
    const highlightedText =
      '<span class="font-bold text-yellow dark:text-softYellow">We\'re</span>&nbsp;<span class="font-bold text-yellow dark:text-softYellow">family</span>';
