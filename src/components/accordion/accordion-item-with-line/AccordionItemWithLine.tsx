  ...
  return (
    <div className={`${!show ? 'hidden' : ''} w-full p-[15px] overflow-hidden`}>
      <button
        ...
        className="w-full flex items-center gap-md pb-md border border-light-gray dark:border-dark-gray border-x-0 border-t-0"
      >
        <Plus
          className={`fill-brand-primary ${open ? 'rotate-45' : ''} transition-all duration-300`}
        />
        <h3
          className={`max-sm:text-[20px] text-light-navy dark:text-dark-navy text-left font-thin tracking-wide ${
            open ? 'font-bold' : ''
          }`}
        >
          {title}
      ...
      >
        <div
          ref={accordionDesc}
          className="body-large text-light-charcoal dark:text-dark-primaryText"
        >
          {description}
        </div>
      </div>
