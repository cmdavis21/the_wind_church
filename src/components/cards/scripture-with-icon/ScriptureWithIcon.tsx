...
const ScriptureWithIcon: React.FC<ScriptureWithIconProps> = ({
  ...
  <div
    ...
    } items-center justify-center gap-sm w-full min-w-[250px] max-w-[350px] dark:text-softWhite`}
  >
    <div className="rounded-full p-2 bg-lightGray/50">
      <Icon className="size-[25px] fill-softBlue" />
    </div>
    <div
      className={`flex flex-col ${!horizontal ? "items-center " : ""} gap-sm`}
    >
      {title && <h5 className="font-bold">{title}</h5>}
      <p className="font-light italic text-charcoal dark:text-softYellow">
        {verse}
      </p>
