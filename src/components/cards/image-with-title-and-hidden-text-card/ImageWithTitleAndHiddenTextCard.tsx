...
const ImageWithTitleAndHiddenTextCard: React.FC<ImageWithTitleAndHiddenTextCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Color Overlay */}
      <div className="absolute w-full h-full rounded-lg bg-gradient-to-t from-primary/40 from-5% via-black/30 via-30% to-black/60 to-90%" />
      ...
      >
        <div className="flex items-center justify-center gap-sm text-white">
          {Icon &&
            (typeof Icon === 'function' ? <Icon className="fill-white size-[25px]" /> : Icon)}
          <h2>{title}</h2>
        </div>
